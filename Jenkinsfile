node {
       stage('SCM Checkout') { 
            ...
            url: 'https://github.com/rohanjoshi95/backend.git']]])
       }
        stage ('build package')  {
            sh '''
            gradle build
        ...
        stage("Build Docker Image"){
            ...
            def customImage = docker.build("rohanjoshi95/backend:latest")
            customImage.push()
            }
        }
        stage('Deployment on Dev Environment'){
		    sh '''
			scp /var/lib/jenkins/workspace/backend/deployment ec2-user@198.60.105.2:/backend/deployment
			'''
		    sshagent (credentials: ['deploy-dev']) {
				sh 'ssh -o StrictHostKeyChecking=no -l ec2-user@198.60.105.2'
				sh kubectl apply -f /backend/deployment/backend-deployment.yml
			}
        }
        ...
        stage('Deployment on Prod Environment'){
		    sh '''
			scp /var/lib/jenkins/workspace/backend/deployment ec2-user@198.60.108.2:/backend/deployment
			'''
